...
@app.get(
    ...
    description="Generate the speech from text. Currently, it supports Chinese(zh) and English(en).",
)
...
@app.post(
    "/recognize_speech",
    summary="Speech to Text Engine",
    description="Recognize the speech from an audio file. Currently, it supports Chinese(zh) and English(en).",
)
def speech_to_text_recognize(file: UploadFile = File(...), lang: str = "zh"):
    if not os.path.exists(TEMP_CACHE_PATH):
        os.makedirs(TEMP_CACHE_PATH)
    ret, val = save_audio_file(
        file=file
    )  # return file_name when successfully save the audio files
    if not ret:
        return {"text": "", "msg": val}
    try:
...
@app.post(
    "/do_speech_training",
    summary="Speech Training Engine",
    description="Automatically score your audio speech file. Currently, it supports Chinese(zh) and English(en).",
)
